    ...
    return (
        <div className="flex ">
            <button
                onClick={onClick}
                disabled={disabled}
                className={` md:hover:text-white ${disabled ? "text-gray-500 hover:bg-transparent" : "md:hover:bg-gray-700"} overflow-hidden flex-1 flex-col items-center  p-2 mx-4 text-left transition duration-75 rounded-lg  group md:hover:bg-blue-300  ${isSelected ? 'bg-gray-700 text-white' : ''}`}
            >
...
const CategorySelector: React.FC<CategoryProps> = ({ category, onClick, setIsDegreeSelected }) => {
        ...
        <li className="border-1 rounded-lg border-gray-500">
            ...
            >
                <span className="flex-1 ms-3 text-left rtl:text-right whitespace-nowrap">
                    {/* {(category instanceof DegreeCategory) ? (isDegreeSelected ? degreeTitle : category.title) : } */}
                    {category.title}
                </span>
                <DropdownIcon />
            </button>
